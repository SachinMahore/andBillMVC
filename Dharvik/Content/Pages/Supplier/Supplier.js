...
var saveUpdateAccount = function () {
    ...
    if (supplierName == "") {
        msg += " Please enter Supplier Name.<br />";
    }
    if (address == "") {
            ...
            if ($.trim(response.error) != "") {
                ...
                if (response.SupplierId != 0) {
                    if ($("#hndAccountID").val() == "0") {
                        ...
                        $("#txtSupplierName").val(firstname + ' ' + lastname);
                        $.alert({
                            title: 'Alert!',
    ...
    var fillAccountTable = function (PN, SO) {
        ...
        $.ajax({
            url: '/Supplier/GetSuppAccountsList',
            type: "post",
            data: JSON.stringify(model),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                ...
                if ($.trim(response.error) != "") {
                    ...
                    $.each(response, function (index, elementValue) {
                        var html = '';
                        html += '<tr  class="gradeX" data-value="' + elementValue.SupplierId + '">';
                        html += '<td  class="sorting_1 pd-vehicletblText"><a href="#"  onclick="editAccount(' + elementValue.SupplierId + ');">' + elementValue.SupplierName + '</a></td>';
                        html += '<td class="pd-vehicletblText"><a href="#"  onclick="editAccount(' + elementValue.SupplierId + ');"> ' + elementValue.SupplierId + '</td>';
