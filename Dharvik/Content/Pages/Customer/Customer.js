...
$(document).ready(function () {
    ...
    getInvoiceDetails($("#hndAccountID").val());
...
var getInvoiceDetails = function (accountID) {
    debugger
    //var accountID = $("#hndAccountID").val();
    var param = { AccountID: accountID }

    $.ajax({
        url: '/Customer/GetInvoiceDetails',
        type: "post",
        data: JSON.stringify(param),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if ($.trim(response.error) != "") {
                ...
                $.each(response, function (index, elementValue) {
                    var html = '';
                    html += '<tr data-value="' + elementValue.AutoInvoiceID + '">';
                    html += '<td >' + elementValue.AutoInvoiceID + '</td>';
                    html += '<td>' + parseFloat(elementValue.TotalAmt).toFixed(2) + '</td>';
                    html += '<td>' + parseFloat(elementValue.Advance).toFixed(2) + '</td>';
                    html += '<td>' + parseFloat(elementValue.Balance).toFixed(2) + '</td>';
                    html += '<td>' + parseFloat(elementValue.InvVat).toFixed(2) + '</td>';
                    html += '<td>' + parseFloat(elementValue.InvDiscount).toFixed(2) + '</td>';
                    html += '<td> ' + elementValue.InvoiceDateText + '</td>';
                    html += '<td> ' + elementValue.Person + '</td>';
                    html += '<td><div><button onclick="editDecal(' + elementValue.AutoInvoiceID + ')"><i class="icon-pencil" ></i></button></div></td>';
                    html += '</tr>';
                    $("#tblInvoiceList>tbody").append(html);
                });
            }
        }
    });
