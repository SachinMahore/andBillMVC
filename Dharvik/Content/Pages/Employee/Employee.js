...
var saveUpdateAccount = function () {
    ...
    var datails = $("#txtDetails").val();
   

    if (empName == "") {
        msg += " Please enter Employee Name.<br />";
    }
    if (address == "") {
            ...
            if ($.trim(response.error) != "") {
                ...
                if (response.EmpId != 0) {
                    if ($("#hndAccountID").val() == "0") {
                        ...
                        $("#txtEmpName").val(firstname + ' ' + lastname);
                        $.alert({
                            title: 'Alert!',
...
var newAccount = function () {
    window.location = '/Employee/AddEdit';
}
...
var fillAccountTable = function (PN, SO) {
    ...
    $.ajax({
        url: '/Employee/GetEmpAccountsList',
        type: "post",
        data: JSON.stringify(model),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            ...
            if ($.trim(response.error) != "") {
                ...
                $.each(response, function (index, elementValue) {
                    var html = '';
                    html += '<tr  class="gradeX" data-value="' + elementValue.EmpId + '">';
                    html += '<td  class="sorting_1 pd-vehicletblText"><a href="#"  onclick="editAccount(' + elementValue.EmpId + ');">' + elementValue.EmpName + '</a></td>';
                    html += '<td class="pd-vehicletblText"><a href="#"  onclick="editAccount(' + elementValue.EmpId + ');"> ' + elementValue.EmpId + '</td>';
